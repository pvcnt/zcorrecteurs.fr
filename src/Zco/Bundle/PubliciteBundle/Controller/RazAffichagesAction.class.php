 ...
 * d'impressions d'une publicité pour une date donnée.
 *
 * @author vincent1870 <vincent@zcorrecteurs.fr>
 */
class RazAffichagesAction
{
	public function execute()
	{
		...
		if (!empty($_GET['id']) && is_numeric($_GET['id']) && !empty($_GET['date']))
		{
			...
			if($publicite === false)
				return redirect(6, 'gestion.html', MSG_ERROR);

			if (strtotime($_GET['date']) === false)
				return redirect(8, 'publicite-'.$publicite['id'].'.html', MSG_ERROR);

			$publicite->razAffichages($_GET['date']);
			return redirect(11, 'publicite-'.$publicite['id'].'.html');
		}
		else
