...
//
$PHP_SELF= isset($_SERVER['REQUEST_URI']) ? htmlentities(strip_tags($_SERVER['REQUEST_URI'],''), ENT_QUOTES, 'UTF-8') : '';
...
if (isset($MYREQUEST['IMG']))
{
	...
	function fill_box($col_black, $im, $x, $y, $w, $h, $color1, $color2,$text='',$placeindex='') {
		$x1=$x+$w-1;
		$y1=$y+$h-1;
	...
	case 2:
		...
		fill_box($col_black,$image, 30,$size,50,-$a*($size-21)/$s,$col_black,$col_green,sprintf("%.1f%%",$cache['num_hits']*100/$s));
		fill_box($col_black,$image,130,$size,50,-max(4,($s-$a)*($size-21)/$s),$col_black,$col_red,sprintf("%.1f%%",$cache['num_misses']*100/$s));
		break;
		...
		for($i=0; $i<$mem['num_seg']; $i++) {
			...
			foreach($free as $block) {
				if($block['offset']!=$ptr) {       // Used block
					...
					if ($h>0) {
                                                $j++;
						if($j<75) fill_box($col_black,$image,$x,$y,50,$h,$col_black,$col_red,bsize($block['offset']-$ptr),$j);
                                                else fill_box($col_black,$image,$x,$y,50,$h,$col_black,$col_red);
                                        }
					$y+=$h;
				}
				...
				if ($h>0) {
                                        $j++;
					if($j<75) fill_box($col_black, $image,$x,$y,50,$h,$col_black,$col_green,bsize($block['size']),$j);
					else fill_box($col_black, $image,$x,$y,50,$h,$col_black,$col_green);
                                }
				$y+=$h;
				$ptr = $block['offset']+$block['size'];
			}
			if ($ptr < $mem['seg_size']) { // memory at the end
				...
				if ($h > 0) {
					fill_box($col_black, $image,$x,$y,50,$h,$col_black,$col_red,bsize($mem['seg_size']-$ptr),$j++);
				}
			}
		}
		break;
...
function sortheader($MYREQUEST, $MY_SELF_WO_SORT, $key,$name,$extra='') {
...
// create menu entry
function menu_entry($MYREQUEST,$MY_SELF,$ob,$title) {
	if ($MYREQUEST['OB']!=$ob) {
...
echo
	menu_entry($MYREQUEST,$MY_SELF,1,'View Host Stats'),
	menu_entry($MYREQUEST,$MY_SELF,2,'System Cache Entries');
if ($AUTHENTICATED) {
	echo menu_entry($MYREQUEST,$MY_SELF,4,'Per-Directory Entries');
}
echo
	menu_entry($MYREQUEST,$MY_SELF,3,'User Cache Entries'),
	menu_entry($MYREQUEST,$MY_SELF,9,'Version Check');
...
EOB;
	...
	echo   '<tr class=tr-0><td class=td-0>Uptime</td><td>',duration($time,$cache['start_time']),'</td></tr>';
...
EOB;
  ...
  echo
		'<div class="info"><table cellspacing=0><tbody>',
		'<tr>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'S',$fieldheading,  "&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'H','Hits',         "&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'Z','Size',         "&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'A','Last accessed',"&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'M','Last modified',"&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'C','Created at',   "&OB=".$MYREQUEST['OB']),'</th>';

	if($fieldname=='info') {
		$cols+=2;
		 echo '<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'T','Timeout',"&OB=".$MYREQUEST['OB']),'</th>';
	}
	echo '<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'D','Deleted at',"&OB=".$MYREQUEST['OB']),'</th></tr>';
...
EOB;
	echo
		...
		'<div class="info"><table cellspacing=0><tbody>',
		'<tr>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'S','Directory Name',	"&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'T','Number of Files',"&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'H','Total Hits',	"&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'Z','Total Size',	"&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'C','Avg. Hits',	"&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader($MYREQUEST, $MY_SELF_WO_SORT, 'A','Avg. Size',	"&OB=".$MYREQUEST['OB']),'</th>',
		'</tr>';
