...
class DefaultController extends Controller
{
    ...
    public function accountsAction(Request $request)
    {
        ...
        if ($request->getMethod() === 'POST' && $request->request->has('default_account')) {
            ...
            return redirect('Le compte par défaut a été modifié.', $this->generateUrl('zco_twitter_accounts'));
        }
    ...
    public function addAccountAction(Request $request)
    {
        ...
        if ($request->query->has('token') && $request->query->get('token') == $_SESSION['token']) {
            if (Doctrine_Core::getTable('TwitterCompte')->add()) {
                return redirect('Le compte a bien été ajouté.', $this->generateUrl('zco_twitter_accounts'));
            }

            return redirect('Une erreur s\'est produite pendant l\'ajout du compte.', $this->generateUrl('zco_twitter_addAccount'), MSG_ERROR);
        }
    ...
    public function deleteAccountAction(Request $request, $id)
    {
        ...
        if ($request->request->has('confirm')) {
            if ($r = zCorrecteurs::verifierToken())
                return $r;
    ...
    public function newTweetAction(Request $request, $mention = null)
    {
        ...
        if ($mention && !($mention  = Doctrine_Core::getTable('TwitterMention')->find($mention))) {
            return redirect('Cette mention n\'existe pas.', $this->generateUrl('zco_twitter_mentions'), MSG_ERROR);
        }
        ...
        if ($request->request->has('compte') && $request->request->has('tweet')) {
            ...
            return redirect('Une erreur s\'est produite pendant l\'ajout du tweet.', $this->generateUrl('zco_twitter_newTweet'), MSG_ERROR);
        }
