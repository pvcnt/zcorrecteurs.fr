...
Form.MultipleFileInput = new Class({
	...
	add: function(file){
		...
		var img = new Element('img')
		    .set('alt', 'Image')
		    /*.twipsy({
    	        title: function(){ return '<strong>Type :</strong> ' + file.type + '<br />' + '<strong>Poids :</strong> ' + (file.size/1000).format() + ' Ko'; },
    	        html: true,
    	        placement: 'left',
    	    })*/;
    	var actions = new Element('p')
		...
		var div = new Element('div')
		    .addClass('thumbnail')
		    .grab(new Element('h5').set('html', file.name))
		    .grab(img)
		    .grab(actions);
